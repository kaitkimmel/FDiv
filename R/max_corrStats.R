...
AIC(mod, mod1,mod2, mod3, mod4) ## all within 2 AIC
anova(mod1, mod4) # no sig dif use mod4
fevemod_black <- lme(FEve ~ 1, random = ~1|sev.blackplots,  data = sev.black[-which(is.na(sev.black$FEve)),], correlation = corCompSymm(form = ~ 1|sev.blackplots))
...
AIC(mod, mod1,mod2, mod3, mod4) #mod2
anova(mod3, mod1) #right at cutoff. 
