...
# Testing for best functional form between linear, quadratic, null
### FRic
...
fricmod_blue <- lme(FRic ~ n_trait + I(n_trait^2), random = ~1|sev.blueplots,  data = sev.blue[-which(is.na(sev.blue$FRic)),], correlation = corCompSymm(form = ~ 1|sev.blueplots))
...
AIC(mod, mod1, mod4) #mod1 best fit
raoqmod_blue <- lme(RaoQ ~ n_trait + I(n_trait^2), random = ~1|sev.blueplots,  data = sev.blue, correlation = corCompSymm(form = ~ 1|sev.blueplots))
...
###### Fit Metric ~ N_trait models
# Plot will be fit as a random effect
# Testing for best functional form between linear, quadratic
### FRic
...
AIC(mod, mod1, mod4) ### mod1
fricmod_black <- lme(FRic ~ n_trait + I(n_trait^2), random = ~1|sev.blackplots,  data = sev.black[-which(is.na(sev.black$FRic)),], correlation = corCompSymm(form = ~ 1|sev.blackplots))
...
AIC(mod, mod1, mod4) #mod1 best fit
raoqmod_black <- lme(RaoQ ~ n_trait + I(n_trait^2), random = ~1|sev.blackplots,  data = sev.black, correlation = corCompSymm(form = ~ 1|sev.blackplots))
...
AIC(mod, mod1, mod4) ### mod
kde.evennessmod_cdr1 <- lme(kde.evenness ~ n_trait, random = ~1|Plot,  data = cdr.1, correlation = corCompSymm(form = ~ 1|Plot))
...
AIC(mod, mod1, mod4) #mod best fit
fdivmod_cdr3 <- lme(FDiv ~ n_trait, random = ~1|Plot,  data = cdr.3, method = "ML", correlation = corCompSymm(form = ~ 1|Plot))
